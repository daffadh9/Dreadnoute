...
import { Mail, Lock, ArrowRight, Skull, Loader2 } from "lucide-react";
...
function InnerAuth() {
  ...
  useEffect(() => {
    ...
    const checkUser = async () => {
      ...
      const needsIntro = searchParams.get("intro") === "true";
      
      if (session) {
        if (needsIntro || !hasSeenIntro) {
          setShowIntro(true);
        } else {
          router.push("/");
        }
    ...
    checkUser();
  }, [router, searchParams, hasSeenIntro]);
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes("Invalid login credentials")) {
          ...
          setError("Pendaftaran berhasil! Silakan cek email kamu untuk konfirmasi.");
        } else {
          throw error;
        }
      } else {
              ...
              <form onSubmit={handleSummon} className="space-y-10 relative z-10">
                ...
                {error && (
                  <motion.div 
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className="p-4 bg-red-500/10 border border-red-500/30 rounded-xl text-[10px] text-red-500 font-bold uppercase tracking-widest text-center"
                  >
                    {error}
                  </motion.div>
                )}
                ...
                <motion.button 
                  ...
                  type="submit"
                  disabled={loading}
                  className="w-full bg-[#8b0000] disabled:bg-red-950/50 disabled:text-zinc-500 text-white py-6 rounded-2xl font-bold uppercase tracking-[0.5em] text-[13px] border border-red-500 shadow-[0_0_20px_rgba(255,0,0,0.5)] transition-all flex items-center justify-center gap-4 relative overflow-hidden group mt-12"
                >
                  <div className="absolute inset-0 bg-red-500 opacity-0 group-hover:opacity-10 transition-opacity" />
                  <span className="relative z-10">{loading ? <Loader2 className="animate-spin" size={20} /> : "YA, SAYA BERANI"}</span>
                  {!loading && <ArrowRight size={22} className="relative z-10 group-hover:translate-x-2 transition-transform opacity-70" />}
                </motion.button>
              </form>
              ...
              {isLocal && (
                <div className="mt-8 flex flex-col items-center gap-4">
                  ...
                  <motion.button
                    ...
                    className="w-full py-4 bg-zinc-900 border border-zinc-800 text-zinc-500 rounded-2xl text-[10px] font-black uppercase tracking-[0.4em] hover:text-white hover:border-white/20 transition-all flex items-center justify-center gap-3"
                  >
                    🛠️ Buka Akses Lokal (Dev Mode)
                  </motion.button>
                </div>
              )}
            </motion.div>
